...
console.log('=== TEST 1: READ /root/victim-home/.ssh/id_rsa (outside the workspace) from inside the real sandbox ===');
const readResult = spawnSync('bwrap', [...bwrapArgs, '--', 'cat', '/root/victim-home/.ssh/id_rsa'], { encoding: 'utf8' });
console.log('exit code:', readResult.status);
console.log('stdout:', readResult.stdout);
console.log('stderr:', readResult.stderr);

console.log();
console.log('=== TEST 2 (control): WRITE to /root/victim-home/.ssh/id_rsa from inside the same sandbox ===');
const writeResult = spawnSync('bwrap', [...bwrapArgs, '--', 'sh', '-c', 'echo pwned > /root/victim-home/.ssh/id_rsa'], { encoding: 'utf8' });
...
const fileStillIntact = fs.readFileSync('/root/victim-home/.ssh/id_rsa', 'utf8').includes('SIMULATED_REAL_SECRET_SSH_KEY');
